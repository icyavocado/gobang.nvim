...
local function open_floating_window()
  local floating_window_scaling_factor = vim.g.gobang_floating_window_scaling_factor

  -- Why is this required?
  -- vim.g.gobang_floating_window_scaling_factor returns different types if the value is an integer or float
  ...
  if status and vim.g.gobang_floating_window_use_plenary and vim.g.gobang_floating_window_use_plenary ~= 0 then
    local ret = plenary.percentage_range_window(
      floating_window_scaling_factor,
      floating_window_scaling_factor,
      { winblend = vim.g.gobang_floating_window_winblend }
    )
  ...
  local window_chars = vim.g.gobang_floating_window_border_chars
  ...
  vim.api.nvim_set_hl(0, "gobangBorder", { link = "Normal", default = true })
  vim.cmd("set winhl=NormalFloat:gobangBorder")

  -- create a unlisted scratch buffer
  if gobang_BUFFER == nil or vim.fn.bufwinnr(gobang_BUFFER) == -1 then
    gobang_BUFFER = api.nvim_create_buf(false, true)
  else
    gobang_LOADED = true
  end
  -- create file window, enter the window, and use the options defined in opts
  local win = api.nvim_open_win(gobang_BUFFER, true, opts)

  vim.bo[gobang_BUFFER].filetype = "gobang"

  vim.cmd("setlocal bufhidden=hide")
  vim.cmd("setlocal nocursorcolumn")
  vim.api.nvim_set_hl(0, "gobangFloat", { link = "Normal", default = true })
  vim.cmd("setlocal winhl=NormalFloat:gobangFloat")
  vim.cmd("set winblend=" .. vim.g.gobang_floating_window_winblend)
